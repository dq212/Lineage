...
var src = './src',
  ...
  port = 8000 + Math.floor(Math.random() * 500),
...
(function (tasks) {
  tasks.forEach(function (task) {
    ...
    gulp.task(
      'moveGeneratedImages' + task.append,
      ['sass' + task.append],
      function () {
        return gulp
...
gulp.task('watch', function () {
  ...
  gulp.watch(
    ['./sass/*.scss'],
    ['sass', 'moveGeneratedImages', 'cleanSprites']
  );
  gulp.watch([src + '/js/*.js'], ['lint']);
  gulp.watch(
    ['src/sprites/**/*'],
    ['sassForce', 'moveGeneratedImagesForce', 'cleanSpritesForce']
  );
});
...
gulp.task('copyStatic', function () {
  gulp
    ...
    .pipe(rename('PG_KnowBefore_320x50_STATIC.jpg'))
...
gulp.task(
  'compress',
  ['clean', 'copy', 'sass', 'inline', 'replace', 'htmlmin', 'imageCompress'],
  function () {
    return gulp
      .src(dist + '/*')
      .pipe(zip('PG_KnowBefore_320x50_HTML.zip'))
      .pipe(gulp.dest('../HTML/'));
  }
);
